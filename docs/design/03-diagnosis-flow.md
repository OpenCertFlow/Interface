...
| AI 워커 반복 무응답 (회로 열림) | 진단 성공, 호출 생략 후 즉시 위 폴백 | 위와 동일 — 타임아웃 대기 없이 바로 응답 |
...
위 폴백은 요청 단위라, 워커가 **무응답**(행·과부하·모델 로딩 중)으로 계속 죽어 있으면
요청마다 타임아웃(2초+5초)을 처음부터 다시 기다립니다. 회로 차단기가 이 반복을 끊습니다.

- **열림**: 최근 10건 중 절반이 실패하면(최소 5건 집계) 워커를 부르지 않고 즉시 위 폴백으로
  갑니다. 결과는 같고 타임아웃 대기만 사라집니다.
- **자동 복구**: 열린 지 30초가 지나면 시험 호출 3건을 통과시켜 보고, 성공하면 스스로
  닫힙니다. 사람이 개입하지 않습니다.
- RAG와 LLM은 **별개 회로**입니다. 서로 다른 엔드포인트라 한쪽 장애가 다른 쪽을 차단하면
  안 됩니다.
- 관리자 폴백 스위치(F-WADM-020)와는 용도가 다릅니다. 스위치는 사람이 미리 아는 상황에서
  선제적으로 끄는 것, 차단기는 아무도 모를 때 자동으로 감지하는 것이며, 스위치가 켜져
  있으면 호출 자체가 없어 차단기는 개입하지 않습니다.
- **재시도는 하지 않습니다.** 진단은 사용자가 기다리는 동기 요청이라 재시도는 최악 대기를
  늘립니다. 차단기는 대기를 없애는 방향입니다.
...
| `POST` | `/api/v1/diagnoses/{id}/simulations` | **반사실 시뮬레이션** — 가정을 적용한 결과와 원본 대비 델타 |
| `GET` | `/api/v1/diagnoses/{id}/remediation-plan` | **최소 보완 경로** — 목표 준비도 도달에 필요한 최소 서류 조합 |
...
## 3.7 시뮬레이션 — 결정론이 만들어 낸 기능

```mermaid
flowchart LR
    BASE(["저장된 진단<br/>(점수 · 체크리스트 · 후보)"]) --> ADJ["가정 적용<br/>ProfileAdjustment"]
    ADJ --> EVAL["룰 재평가 + 점수 재산정<br/>(순수 함수, 수 ms)"]
    EVAL --> DELTA["원본과 비교<br/>· 점수 변화(%p)<br/>· 후보 증감<br/>· 요구 서류 증감"]
    DELTA --> OUT(["시뮬레이션 결과<br/>저장하지 않음"])

    style OUT fill:#e8f5e9,stroke:#2e7d32
```

`POST /simulations`는 **RAG 검색도 LLM 문장화도 하지 않습니다.** 결정론 영역(①룰셋 로드 → ②룰 평가·점수 산정)만 다시 돌리므로 응답이 수 밀리초 안에 끝나고, 화면에서 체크박스를 켤 때마다 즉시 반응할 수 있습니다.

**이 기능은 ADR-0003의 직접적 산물입니다.** 판정이 결정론적이기 때문에 "이 서류를 준비하면 준비도가 정확히 몇 %가 되는가"에 확정적으로 답할 수 있습니다. 판정을 LLM에 맡겼다면 "아마 오를 것"이라는 추측밖에 할 수 없습니다.

**결과를 저장하지 않는 이유**는 성능이 아니라 기록의 무결성입니다. 원본 진단은 특정 시점의 룰셋 버전으로 확정된 기록이고, 가정이 그것을 덮어쓰면 "이 진단은 언제 무엇을 근거로 나왔는가"에 답할 수 없게 됩니다.

### 최소 보완 경로

기존 보완 우선순위(3.3)는 누락 서류를 가중치 내림차순으로 나열한 것입니다. 그것은 **무엇부터**는 알려주지만 **어디까지**는 알려주지 못합니다. `GET /remediation-plan?targetScore=80`은 목표를 받아 "몇 개만 준비하면 되는지"를 끊어 줍니다.

가중치가 큰 서류부터 고르는 그리디가 최적해입니다 — *k*개로 도달 가능하다면 가장 큰 *k*개의 합은 어떤 *k*개 조합의 합보다 작지 않기 때문입니다. 배낭 문제로 오해해 DP를 끌어올 필요가 없습니다.
