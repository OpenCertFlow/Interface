...
class ConsultingWorkflowIntegrationTest {
    ...
    private final String consultantId = io.opencertflow.support.TestIds.nextString();

    private String tokenFor(Role role, String id) {
        User user = User.reconstitute(
                UserId.of(Long.parseLong(id)), Email.of(role + "@opencertflow.local"), null,
    ...
    void 권한이_없으면_접근할_수_없다() {
        ...
        webTestClient.get().uri("/api/v1/consulting/leads")
                .header("Authorization", "Bearer " + tokenFor(Role.USER, io.opencertflow.support.TestIds.nextString()))
                .exchange()
    ...
    void 로그인_접수는_내_상담으로_조회된다() {
        String userToken = tokenFor(Role.USER, io.opencertflow.support.TestIds.nextString());
