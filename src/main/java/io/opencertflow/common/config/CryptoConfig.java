...
import io.opencertflow.common.adapter.out.crypto.AesGcmTextEncryptor;
 ...
 * {@code OPENCERTFLOW_SECURITY_ENCRYPTION_KEY} 환경변수로 고정 키를 주입해야 한다.
 */
@Configuration
...
public class CryptoConfig {
    ...
    private byte[] resolveKey(String configured) {
        ...
        log.warn("개인정보 암호화: 설정된 키가 없어 임시 키를 생성했습니다. 재시작 시 이전 데이터를 "
                + "복호화할 수 없습니다. 운영에서는 opencertflow.security.encryption-key를 반드시 설정하세요.");
        return ephemeral;
    }
}
