package io.opencertflow.diagnosis.adapter.in.scheduler;
...
public class DocumentFreshnessScheduler {
    ...
    public DocumentFreshnessScheduler(
            ...
            @Value("${opencertflow.document-freshness.enabled:false}") boolean enabled) {
        this.monitorUseCase = monitorUseCase;
        this.enabled = enabled;
    }

    @Scheduled(cron = "${opencertflow.document-freshness.cron:0 0 4 * * MON}")
