 ...
 *
 * @param bodyContactType            신체에 닿는 방식(F-APP-014). NONE이면 비접촉
 * @param controllerStatus           온도조절기 유무(있음/없음/모름, F-APP-016)
 * @param adjustmentSteps            온도 조절 단계 수(예: 3단). 조절기가 PRESENT일 때만 값이 있다
...
public record HeatingSpec(
    ...
    public HeatingSpec {
        ...
        // 조절 단계는 조절기가 있을 때만 의미가 있다. "모름/없음"인데 단계 수가 있으면 모순이다.
        if (controllerStatus == ControllerStatus.PRESENT) {
            if (adjustmentSteps == null) {
                throw new IllegalArgumentException("온도조절기가 있으면 조절 단계 수를 입력해야 합니다.");
            }
